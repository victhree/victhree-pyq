...
(function () {
  ...
  var GREETINGS = [
    ["Welcome back, {name}.", "Every solved paper is one step closer to the Academy."],
    ["Good to see you, {name}.", "Consistency with PYQs is what separates selection from regret."],
    ["{name}, let's put in the reps.", "The questions repeat; make sure your preparation does too."],
    ["Sharp and steady, {name}.", "Master the past papers and the exam holds no surprises."],
    ["Back at it, {name}.", "A few honest questions today beat a hundred someday."],
    ["{name}, the cutoff rewards the prepared.", "Revise, attempt, review, repeat."],
    ["Keep the momentum, {name}.", "Officers are made in quiet hours of practice like these."],
    ["Rise a little sharper, {name}.", "That is what the CDS really measures."]
  ];

  function firstName() {
    var raw = "";
    try { raw = (JSON.parse(localStorage.getItem("vtpyq_lead_data") || "{}").name) || ""; } catch (e) {}
    raw = raw.trim();
    if (!raw) return "";
    var f = raw.split(/\s+/)[0];
  ...
  function renderGreeting() {
    ...
    var i = 0;
    try { i = parseInt(localStorage.getItem("vtpyq_greet_i") || "0", 10) || 0; } catch (e) {}
    var msg = GREETINGS[i % GREETINGS.length];
    try { localStorage.setItem("vtpyq_greet_i", String((i + 1) % GREETINGS.length)); } catch (e) {}
  ...
  function show() {
    ...
    form.addEventListener("submit", function (ev) {
      ...
      try { localStorage.setItem("vtpyq_lead_data", JSON.stringify(data)); } catch (e) {}
