...
function intro() {
  ...
  $('quiz').innerHTML = `
    <div class="quiz-intro">
      ...
      <h2>${scopeTitle}</h2>
      <p>${scopeText} Untimed — pick your answers, then <b>Submit</b> to see your score with the correct answers and explanations.</p>
...
function submitQuiz() {
  ...
  const bar = `
    <div class="scorecard">
      ...
      <div style="margin-top:14px"><button class="btn" id="again">Take a new ${current.length}-question quiz</button></div>
    </div>`;
  const qlist = $('qlist');
